...
QTSS_Error RereadPrefs()
{
	...
	g_StbUserAgent = QTSSModuleUtils::GetStringAttribute(sPrefs, "stb_useragent", "Defalut"); //zlj for nds
...
UInt32 Stb_User_Agent(char *UserAgentStr){
	char StrTemp[512] = {'\0'};                                                                                                                                                                                                             
	qtss_printf("zlj --g_StbUserAgent:%s",g_StbUserAgent);
	if (g_StbUserAgent != NULL){
		strncpy(StrTemp,g_StbUserAgent,strlen(g_StbUserAgent));
		StrTemp[strlen(StrTemp) + 1] = '\0';
		char *UserAgentArr[512] = {'\0'};
		char UserAgentLen = 0;
		char UserAgentLenIndex = 0;
		if (-1 == StrExplode(StrTemp,":",UserAgentArr,&UserAgentLen)){
			qtss_printf("zlj --stb useragent match of xml file faild error stb_module is default\n");
			UserAgentLenIndex = 0;
		} else{
			for (UserAgentLenIndex = 0; UserAgentLenIndex < UserAgentLen; UserAgentLenIndex++){
				if (NULL != strstr(UserAgentArr[UserAgentLenIndex],UserAgentStr)){
					qtss_printf("zlj ---stb_module match success this stb useragent is :%s,transport_mode:%d\n",UserAgentArr[UserAgentLenIndex],UserAgentLenIndex);
					break;
				}
			}
			if (UserAgentLenIndex == UserAgentLen){
				qtss_printf("zlj --stb useragent match faild error stb_module is default\n");
				UserAgentLenIndex = 0;
			}
		}
		//((RTSPRequestInterface*)inParams->inRTSPRequest)->SetTransportUserAgentMode((UInt32)UserAgentLenIndex);
		return (UInt32)UserAgentLenIndex;
	} else{
		return (UInt32)0;
		//((RTSPRequestInterface*)inParams->inRTSPRequest)->SetTransportUserAgentMode(0);
	}
}
...
QTSS_Error  generate_sdp(char *vid,UInt32 l_callid,char *sdp_buf,Bool16 isCamera,char *src_host,UInt32 *src_port,char *local_host,QTSS_StandardRTSP_Params* inParams)
{
	...
	//zlj add 20161208
	char UserAgentTempBuff[256] = {'\0'};
	StrPtrLen UserAgentStr(UserAgentTempBuff,sizeof(UserAgentTempBuff) - 1);
	(void)QTSS_GetValue(inParams->inClientSession, qtssCliSesFirstUserAgent, 0, UserAgentStr.Ptr, &UserAgentStr.Len);//for nds stb TransportMode                                                                                            
	qtss_printf("\n---------User-Agent:%s\n",UserAgentStr.Ptr);
	UInt32 T_mod = Stb_User_Agent(UserAgentStr.Ptr);
...
//zlj add 20161208  transport_mode 获取
char UserAgentTempBuff[256] = {'\0'};
    StrPtrLen UserAgentStr(UserAgentTempBuff,sizeof(UserAgentTempBuff) - 1);
	    (void)QTSS_GetValue(inParams->inClientSession, qtssCliSesFirstUserAgent, 0, UserAgentStr.Ptr, &UserAgentStr.Len);//for nds stb TransportMode
		    qtss_printf("\n---------User-Agent:%s\n",UserAgentStr.Ptr);
			    ((RTSPRequestInterface*)inParams->inRTSPRequest)->SetTransportUserAgentMode(Stb_User_Agent(UserAgentStr.Ptr));
			//end transport_mode 获取
...
{   
	...
	if (theSessionRef == NULL)
	{
		...
		//zlj add for nds
		/**/
		if (Stb_User_Agent(UserAgentStr.Ptr) == SC_NDS_MODULE){
			//zlj add for nds
			OSRef* theSessionRef = sSessionMap->Resolve(inPath);
